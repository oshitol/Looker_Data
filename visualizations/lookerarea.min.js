...
const visObject={create:function(a,e){a.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(a,e,i,t,n,r){const l=t.fields.dimension_like,o=t.fields.measure_like[0],s=[];a.forEach(function(t){const n=[];for(const a of l){if(null===t[a.name].value&&!i.show_null_points)break;n.push(t[a.name].value+"")}n.forEach(function(a,e){s.push({index:s.length,name:n.join("-"),value:+t[o.name].value})})});var d,c="";for(d of a){var u=d[t.fields.dimensions[0].name];c+=LookerCharts.Utils.htmlForCell(u)}e.innerHTML=c,console.log("final chart data",s);a=e.clientWidth,e=e.clientHeight;const f=d3.select("#vis").append("svg").attr("width","100%").attr("height","100%").append("g"),p=d3.scaleBand().range([0,a]).domain(s.map(a=>a.index)).padding(1);f.append("g").attr("transform","translate(0,0)").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const h=d3.scaleLinear().domain([0,d3.max(s,a=>a.value)]).range([e,0]);f.append("g").call(d3.axisLeft(h)).attr("display","none"),f.append("path").datum(s).attr("fill",function(a){return 0<s[0].value?"#0D9401":"#EB0C24"}).attr("stroke",function(a){return 0<s[0].value?"#0B7901":"#AB0416"}).attr("stroke-width",1.5).attr("d",d3.area().x(a=>p(a.index)).y0(h(0)).y1(a=>h(a.value))),r()}};looker.plugins.visualizations.add(visObject);
