...
const visObject={options:{first_option:{type:"string",label:"My First Option",default:"Default Value"},second_option:{type:"number",label:"My Second Option",default:42}},create:function(t,e){t.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(t,o,e,a,n,l){console.log("data",t);const i=a.fields.dimension_like,s=a.fields.measure_like[0],r=[];let d=0;t.forEach(function(a){const n=[];for(const t of i){if(null===a[t.name].value&&!e.show_null_points)break;n.push(a[t.name].value+"")}n.forEach(function(t,e){console.log("data",a[s.name].value),d<+a[s.name].value&&(d=+a[s.name].value,o.innerHTML=`<h4>${s.name}</h4><h4>$${d}</h4>`),r.push({index:r.length,name:n.join("-"),value:+a[s.name].value})})}),console.log("final chart data",r);var c={top:30,right:30,bottom:70,left:60},a=o.clientWidth,t=o.clientHeight;const p=d3.select("#vis").append("svg").attr("width",a+c.left+c.right).attr("height",t+c.top+c.bottom).append("g").attr("transform","translate("+c.left+","+c.top+")"),u=d3.scaleBand().range([0,a]).domain(r.map(t=>t.index)).padding(1);p.append("g").attr("transform","translate(0,"+t+")").selectAll("text").attr("transform","translate(-10,10)rotate(-45)");const f=d3.scaleLinear().domain([0,d3.max(r,t=>t.value)]).range([t,0]);p.append("g").call(d3.axisLeft(f)),p.append("path").datum(r).attr("d",d3.area().x(t=>u(t.index)).y0(f(0)).y1(t=>f(t.value))),l()}};looker.plugins.visualizations.add(visObject);
