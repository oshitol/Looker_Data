...
const visObject={create:function(a,e){a.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(a,e,l,t,n,i){const o=t.fields.dimension_like,r=t.fields.measure_like[0];console.log("data",r);const d=[];a.forEach(function(t){const n=[];for(const a of o){if(null===t[a.name].value&&!l.show_null_points)break;n.push(t[a.name].value+"")}n.forEach(function(a,e){d.push({index:d.length,name:n.join("-"),value:+t[r.name].value})})}),e.innerHTML=`<lable>${r.label_short}</lable><br><lable font-weight: bold;>$${d[0].value}</lable><br>`,console.log("final chart data",d);a=e.clientWidth,e=e.clientHeight;const s=d3.select("#vis").append("svg").attr("width","100%").attr("height","100%").append("g").attr("fill","red"),c=d3.scaleBand().range([0,a]).domain(d.map(a=>a.index)).padding(1);s.append("g").attr("transform","translate(0,0)").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const p=d3.scaleLinear().domain([0,d3.max(d,a=>a.value)]).range([e,0]);s.append("g").call(d3.axisLeft(p)).attr("display","none"),s.append("path").datum(d).attr("fill","#cce5df").attr("stroke","#69b3a2").attr("stroke-width",1.5).attr("d",d3.area().x(a=>c(a.index)).y0(p(0)).y1(a=>p(a.value))),i()}};looker.plugins.visualizations.add(visObject);
