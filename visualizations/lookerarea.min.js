...
const visObject={create:function(t,e){t.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(t,e,o,n,a,l){console.log("data",t),console.log("element",e),console.log("config",o),console.log("queryResponse",n),console.log("details",a);const i=n.fields.dimension_like,r=n.fields.measure_like[0],s=[];t.forEach(function(n){const a=[];for(const t of i){if(null===n[t.name].value&&!o.show_null_points)break;a.push(n[t.name].value+"")}a.forEach(function(t,e){s.push({index:s.length,name:a.join("-"),value:+n[r.name].value})})});var d,c="";for(d of t){var u=d[n.fields.dimensions[0].name];(c=LookerCharts.Utils.htmlForCell(u)).onclick=function(t){LookerCharts.Utils.openDrillMenu({links:u.links,event:t})};break}e.innerHTML=`<table>
    <tr>
      <td>${r.label_short}</td>
      <td></td><td></td>
      <td>${n.fields.dimension_like[0].label_short}</td>
    </tr>
    <tr>
      <td style="font-weight:bold">
    ${s[0].value.toLocaleString("en-US",{style:"currency",currency:"USD"})}
      </td>
      <td></td><td></td>
      <td style="font-weight:bold">
    ${c}
    ...
    <br>`;var f={top:30,right:30,bottom:70,left:60},a=e.clientWidth,t=e.clientHeight;const h=d3.select("#vis").append("svg").attr("width",a+f.left+f.right).attr("height",t+f.top+f.bottom).append("g").attr("transform","translate("+f.left+","+f.top+")"),g=d3.scaleBand().range([0,a]).domain(s.map(t=>t.index)).padding(1);h.append("g").attr("transform","translate(0,"+t+")").call(d3.axisBottom(g).tickFormat(t=>s[t].name)).selectAll("text").attr("transform","translate(-10,10)rotate(-45)").style("text-anchor","end");const m=d3.scaleLinear().domain([0,d3.max(s,t=>t.value)]).range([t,0]);h.append("g").call(d3.axisLeft(m)),h.append("path").datum(s).attr("fill",function(t){return 0<s[0].value?"#0D9401":"#EB0C24"}).attr("stroke",function(t){return 0<s[0].value?"#0B7901":"#AB0416"}).attr("stroke-width",1.5).attr("d",d3.area().x(t=>g(t.index)).y0(m(0)).y1(t=>m(t.value)));var p=this;$(e).find(".axis").click(function(){p.trigger("updateConfig",[{axis_hidden:!0}])}),l()}};looker.plugins.visualizations.add(visObject);
