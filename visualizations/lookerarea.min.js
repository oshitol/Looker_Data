...
const visObject={create:function(a,e){a.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(a,e,l,t,n,i){const o=t.fields.dimension_like,r=t.fields.measure_like[0];console.log("data",r);const s=[];a.forEach(function(t){const n=[];for(const a of o){if(null===t[a.name].value&&!l.show_null_points)break;n.push(t[a.name].value+"")}n.forEach(function(a,e){s.push({index:s.length,name:n.join("-"),value:+t[r.name].value})})}),e.innerHTML=`<lable>${r.label_short}</lable><br><lable style="font-weight:bold">$${s[0].value}</lable><br>`,console.log("final chart data",s);a=e.clientWidth,e=e.clientHeight;const d=d3.select("#vis").append("svg").attr("width","100%").attr("height","100%").append("g"),c=d3.scaleBand().range([0,a]).domain(s.map(a=>a.index)).padding(1);d.append("g").attr("transform","translate(0,0)").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const u=d3.scaleLinear().domain([0,d3.max(s,a=>a.value)]).range([e,0]);d.append("g").call(d3.axisLeft(u)).attr("display","none"),d.append("path").datum(s).attr("fill",function(a){return 0<s[0].value?"#16EA03":"#EB0C24"}).attr("stroke",function(a){return 0<s[0].value?"#0B7901":"#AB0416"}).attr("stroke-width",1.5).attr("d",d3.area().x(a=>c(a.index)).y0(u(0)).y1(a=>u(a.value))),i()}};looker.plugins.visualizations.add(visObject);
