...
const visObject={create:function(e,t){e.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(e,t,l,a,n,r){const i=a.fields.dimension_like,o=a.fields.measure_like[0];console.log("data",o);const s=[];e.forEach(function(a){const n=[];for(const e of i){if(null===a[e.name].value&&!l.show_null_points)break;n.push(a[e.name].value+"")}n.forEach(function(e,t){s.push({index:s.length,name:n.join("-"),value:+a[o.name].value})})}),t.innerHTML=`<lable>${o.label_short}</lable>
    <br>
    <lable style="font-weight:bold">${s[0].value.toLocaleString("en-US",{style:"currency",currency:"USD"})}
    </lable>
    <br>`,console.log("final chart data",s);e=t.clientWidth,t=t.clientHeight;const d=d3.select("#vis").append("svg").attr("width","100%").attr("height","100%").append("g"),c=d3.scaleBand().range([0,e]).domain(s.map(e=>e.index)).padding(1);d.append("g").attr("transform","translate(0,0)").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const u=d3.scaleLinear().domain([0,d3.max(s,e=>e.value)]).range([t,0]);d.append("g").call(d3.axisLeft(u)).attr("display","none"),d.append("path").datum(s).attr("fill",function(e){return 0<s[0].value?"#0D9401":"#EB0C24"}).attr("stroke",function(e){return 0<s[0].value?"#0B7901":"#AB0416"}).attr("stroke-width",1.5).attr("d",d3.area().x(e=>c(e.index)).y0(u(0)).y1(e=>u(e.value))),r()}};looker.plugins.visualizations.add(visObject);
