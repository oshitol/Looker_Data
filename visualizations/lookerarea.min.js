...
const visObject={options:{first_option:{type:"string",label:"My First Option",default:"Default Value"},second_option:{type:"number",label:"My Second Option",default:42}},create:function(e,a){e.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(e,l,a,n,t,o){console.log("data",e);const i=n.fields.dimension_like,s=n.fields.measure_like[0],d=[];let r=0;e.forEach(function(n){const t=[];for(const e of i){if(null===n[e.name].value&&!a.show_null_points)break;t.push(n[e.name].value+"")}t.forEach(function(e,a){console.log("data",n[s.name].value),r<+n[s.name].value&&(r=+n[s.name].value,l.innerHTML=`<h4>${s.name}</h4><h4>$${r}</h4>`),d.push({index:d.length,name:t.join("-"),value:+n[s.name].value})})}),console.log("final chart data",d);n=l.clientWidth,e=l.clientHeight;const c=d3.select("#vis").append("svg").append("g"),p=d3.scaleBand().range([0,n]).domain(d.map(e=>e.index)).padding(1);c.append("g").attr("transform","translate(0,"+e+")").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const u=d3.scaleLinear().domain([0,d3.max(d,e=>e.value)]).range([e,0]);c.append("g").call(d3.axisLeft(u)).attr("display","none"),c.append("path").datum(d).attr("fill","#cce5df").attr("stroke","#69b3a2").attr("stroke-width",1.5).attr("d",d3.area().x(e=>p(e.index)).y0(u(0)).y1(e=>u(e.value))),o()}};looker.plugins.visualizations.add(visObject);
