...
const visObject={create:function(n,e){n.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(n,e,i,t,a,l){const r=t.fields.dimension_like,o=t.fields.measure_like[0],s=[];for(d in n.forEach(function(t){const a=[];for(const n of r){if(null===t[n.name].value&&!i.show_null_points)break;a.push(t[n.name].value+"")}a.forEach(function(n,e){s.push({index:s.length,name:a.join("-"),value:+t[o.name].value})})}),n)for(var d in n){var c=n[t.fields.dimensions[0].name];myBuildElementFunction(c).onclick=function(n){LookerCharts.Utils.openDrillMenu({links:c.links,event:n})}}var u="";for(d of n){c=d[t.fields.dimensions[0].name];u+=LookerCharts.Utils.htmlForCell(c)}e.innerHTML=u,console.log("final chart data",s);var f=e.clientWidth,e=e.clientHeight;const p=d3.select("#vis").append("svg").attr("width","100%").attr("height","100%").append("g"),m=d3.scaleBand().range([0,f]).domain(s.map(n=>n.index)).padding(1);p.append("g").attr("transform","translate(0,0)").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const h=d3.scaleLinear().domain([0,d3.max(s,n=>n.value)]).range([e,0]);p.append("g").call(d3.axisLeft(h)).attr("display","none"),p.append("path").datum(s).attr("fill",function(n){return 0<s[0].value?"#0D9401":"#EB0C24"}).attr("stroke",function(n){return 0<s[0].value?"#0B7901":"#AB0416"}).attr("stroke-width",1.5).attr("d",d3.area().x(n=>m(n.index)).y0(h(0)).y1(n=>h(n.value))),l()}};looker.plugins.visualizations.add(visObject);
