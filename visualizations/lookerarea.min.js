...
const visObject={options:{font_size:{type:"string",label:"Font Size",values:[{Large:"large"},{Small:"small"}],display:"radio",default:"large"}},create:function(t,e){t.innerHTML="<h1>Ready to render!</h1>";t.innerHTML=`
    ...
    </style>`;t=t.appendChild(document.createElement("div"));t.className="text-large",this._textElement=t.appendChild(document.createElement("div"))},updateAsync:function(t,l,s,e,a,n){const i=e.fields.dimension_like,o=e.fields.measure_like[0],r=[];let d=0;t.forEach(function(a){const n=[];for(const t of i){if(null===a[t.name].value&&!s.show_null_points)break;n.push(a[t.name].value+"")}n.forEach(function(t,e){d<+a[o.name].value&&(d=+a[o.name].value,l.innerHTML=`<h1>Max : ${d}</h1>`,"small"==s.font_size?l.className="text-small":l.className="text-large"),r.push({index:r.length,name:n.join("-"),value:+a[o.name].value})})}),console.log("final chart data",r);var c={top:30,right:30,bottom:70,left:60},e=l.clientWidth,t=l.clientHeight;const m=d3.select("#vis").append("svg").attr("width",e+c.left+c.right).attr("height",t+c.top+c.bottom).append("g").attr("transform","translate("+c.left+","+c.top+")"),p=d3.scaleBand().range([0,e]).domain(r.map(t=>t.index)).padding(1);m.append("g").attr("transform","translate(0,"+t+")").call(d3.axisBottom(p).tickFormat(t=>r[t].name)).selectAll("text").attr("transform","translate(-10,10)rotate(-45)").style("text-anchor","end");const f=d3.scaleLinear().domain([0,d3.max(r,t=>t.value)]).range([t,0]);m.append("g").call(d3.axisLeft(f)),m.append("path").datum(r).attr("fill","#cce5df").attr("stroke","#69b3a2").attr("stroke-width",1.5).attr("d",d3.area().x(t=>p(t.index)).y0(f(0)).y1(t=>f(t.value))),n()}};looker.plugins.visualizations.add(visObject);
const visObject={options:{first_option:{type:"string",label:"My First Option",default:"Default Value"},second_option:{type:"number",label:"My Second Option",default:42}},create:function(t,e){t.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(t,o,e,a,n,l){const i=a.fields.dimension_like,r=a.fields.measure_like[0],s=[];let d=0;t.forEach(function(a){const n=[];for(const t of i){if(null===a[t.name].value&&!e.show_null_points)break;n.push(a[t.name].value+"")}n.forEach(function(t,e){d<+a[r.name].value&&(d=+a[r.name].value,o.innerHTML=`<h2>${a[r.name]}</h2><br><h2>$${d}</h2>`),s.push({index:s.length,name:n.join("-"),value:+a[r.name].value})})}),console.log("final chart data",s);var c={top:30,right:30,bottom:70,left:60},a=o.clientWidth,t=o.clientHeight;const p=d3.select("#vis").append("svg").attr("width",a+c.left+c.right).attr("height",t+c.top+c.bottom).append("g").attr("transform","translate("+c.left+","+c.top+")"),u=d3.scaleBand().range([0,a]).domain(s.map(t=>t.index)).padding(1);p.append("g").attr("transform","translate(0,"+t+")").call(d3.axisBottom(u).tickFormat(t=>s[t].name)).selectAll("text").attr("transform","translate(-10,10)rotate(-45)").style("text-anchor","end");const f=d3.scaleLinear().domain([0,d3.max(s,t=>t.value)]).range([t,0]);p.append("g").call(d3.axisLeft(f)),p.append("path").datum(s).attr("fill","#cce5df").attr("stroke","#69b3a2").attr("stroke-width",1.5).attr("d",d3.area().x(t=>u(t.index)).y0(f(0)).y1(t=>f(t.value))),l()}};looker.plugins.visualizations.add(visObject);
