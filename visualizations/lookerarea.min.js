...
const visObject={create:function(t,e){t.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(t,e,l,n,a,r){const o=n.fields.dimension_like,i=n.fields.measure_like[0];console.log("data",i);const d=[];t.forEach(function(n){const a=[];for(const t of o){if(null===n[t.name].value&&!l.show_null_points)break;a.push(n[t.name].value+"")}a.forEach(function(t,e){d.push({index:d.length,name:a.join("-"),value:+n[i.name].value})})}),e.innerHTML=`<table>
    <tr>
      <td>${i.label_short}</td>
      <td>${i.label_short}</td>
    </tr>
    ...
    ${d[0].value.toLocaleString("en-US",{style:"currency",currency:"USD"})}
      </td>
      <td style="font-weight:bold">
    ${d[0].value.toLocaleString("en-US",{style:"currency",currency:"USD"})}
      </td>
    ...
    <br>`,console.log("final chart data",d);t=e.clientWidth,e=e.clientHeight;const s=d3.select("#vis").append("svg").attr("width","100%").attr("height","100%").append("g"),c=d3.scaleBand().range([0,t]).domain(d.map(t=>t.index)).padding(1);s.append("g").attr("transform","translate(0,0)").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const u=d3.scaleLinear().domain([0,d3.max(d,t=>t.value)]).range([e,0]);s.append("g").call(d3.axisLeft(u)).attr("display","none"),s.append("path").datum(d).attr("fill",function(t){return 0<d[0].value?"#0D9401":"#EB0C24"}).attr("stroke",function(t){return 0<d[0].value?"#0B7901":"#AB0416"}).attr("stroke-width",1.5).attr("d",d3.area().x(t=>c(t.index)).y0(u(0)).y1(t=>u(t.value))),r()}};looker.plugins.visualizations.add(visObject);
