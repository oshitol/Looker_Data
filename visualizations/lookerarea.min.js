...
const visObject={create:function(t,a){t.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(t,a,i,e,n,r){const l=e.fields.dimension_like,o=e.fields.measure_like[0],s=[];var d;t.forEach(function(e){const n=[];for(const t of l){if(null===e[t.name].value&&!i.show_null_points)break;n.push(e[t.name].value+"")}n.forEach(function(t,a){s.push({index:s.length,name:n.join("-"),value:+e[o.name].value})})});const c="";for(d of t){var u=d[e.fields.dimensions[0].name];c+=LookerCharts.Utils.htmlForCell(u)}a.innerHTML=c,console.log("final chart data",s);t=a.clientWidth,a=a.clientHeight;const f=d3.select("#vis").append("svg").attr("width","100%").attr("height","100%").append("g"),p=d3.scaleBand().range([0,t]).domain(s.map(t=>t.index)).padding(1);f.append("g").attr("transform","translate(0,0)").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const h=d3.scaleLinear().domain([0,d3.max(s,t=>t.value)]).range([a,0]);f.append("g").call(d3.axisLeft(h)).attr("display","none"),f.append("path").datum(s).attr("fill",function(t){return 0<s[0].value?"#0D9401":"#EB0C24"}).attr("stroke",function(t){return 0<s[0].value?"#0B7901":"#AB0416"}).attr("stroke-width",1.5).attr("d",d3.area().x(t=>p(t.index)).y0(h(0)).y1(t=>h(t.value))),r()}};looker.plugins.visualizations.add(visObject);
