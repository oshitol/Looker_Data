...
const visObject={options:{first_option:{type:"string",label:"My First Option",default:"Default Value"},second_option:{type:"number",label:"My Second Option",default:42}},create:function(t,e){t.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(t,n,l,e,a,o){const i=e.fields.dimension_like,r=e.fields.measure_like[0],s=[];t.forEach(function(t){const e=[];for(const a of i){if(null===t[a.name].value&&!l.show_null_points)break;e.push(t[a.name].value+"")}console.log("data",t[r.name].value),n.innerHTML=`<h4>${r.name}</h4><h4>$${t[r.name][0].value}</h4>`,s.push({index:s.length,name:e.join("-"),value:+t[r.name][0].value})}),console.log("final chart data",s);var d={top:0,right:30,bottom:70,left:-145},e=n.clientWidth,t=n.clientHeight;const p=d3.select("#vis").append("svg").attr("width",e+d.left+d.right).attr("height",t+d.top+d.bottom).append("g").attr("transform","translate("+d.left+","+d.top+")"),c=d3.scaleBand().range([0,e]).domain(s.map(t=>t.index)).padding(1);p.append("g").attr("transform","translate(0,"+t+")").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const u=d3.scaleLinear().domain([0,d3.max(s,t=>t.value)]).range([t,0]);p.append("g").call(d3.axisLeft(u)).attr("display","none"),p.append("path").datum(s).attr("fill","#cce5df").attr("stroke","#69b3a2").attr("stroke-width",1.5).attr("d",d3.area().x(t=>c(t.index)).y0(u(0)).y1(t=>u(t.value))),o()}};looker.plugins.visualizations.add(visObject);
