...
const visObject={options:{first_option:{type:"string",label:"My First Option",default:"Default Value"},second_option:{type:"number",label:"My Second Option",default:42}},create:function(t,e){t.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(t,e,o,a,n,l){const i=a.fields.dimension_like,r=a.fields.measure_like[0];console.log("data",r);const s=[];t.forEach(function(a){const n=[];for(const t of i){if(null===a[t.name].value&&!o.show_null_points)break;n.push(a[t.name].value+"")}n.forEach(function(t,e){s.push({index:s.length,name:n.join("-"),value:+a[r.name].value})})}),e.innerHTML=`<h4>${r.label_short}</h4><h4>$${s[0].value}</h4>`,console.log("final chart data",s);a={top:0,right:30,bottom:70,left:-145},t=e.clientWidth,e=e.clientHeight;const d=d3.select("#vis").append("svg").attr("width",t+a.left+a.right).attr("height",e+a.top+a.bottom).append("g").attr("transform","translate("+a.left+","+a.top+")"),c=d3.scaleBand().range([0,t]).domain(s.map(t=>t.index)).padding(1);d.append("g").attr("transform","translate(0,"+e+")").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const p=d3.scaleLinear().domain([0,d3.max(s,t=>t.value)]).range([e,0]);d.append("g").call(d3.axisLeft(p)).attr("display","none"),d.append("path").datum(s).attr("fill","#cce5df").attr("stroke","#69b3a2").attr("stroke-width",1.5).attr("d",d3.area().x(t=>c(t.index)).y0(p(0)).y1(t=>p(t.value))),l()}};looker.plugins.visualizations.add(visObject);
