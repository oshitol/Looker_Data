...
const visObject={create:function(e,n){e.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(e,n,l,t,a,i){const o=t.fields.dimension_like,r=t.fields.measure_like[0],s=[];for(d in e.forEach(function(t){const a=[];for(const e of o){if(null===t[e.name].value&&!l.show_null_points)break;a.push(t[e.name].value+"")}a.forEach(function(e,n){s.push({index:s.length,name:a.join("-"),value:+t[r.name].value})})}),e)for(var d in e){var c=e[t.fields.dimensions[0].name];console.log("cell",c);var u=LookerCharts.Utils.htmlForCell(c);console.log("test",u),myBuildElementFunction(c).onclick=function(e){LookerCharts.Utils.openDrillMenu({links:c.links,event:e})}}var f="";for(d of e){c=d[t.fields.dimensions[0].name];f+=LookerCharts.Utils.htmlForCell(c)}n.innerHTML=f,console.log("final chart data",s);var h=n.clientWidth,n=n.clientHeight;const m=d3.select("#vis").append("svg").attr("width","100%").attr("height","100%").append("g"),p=d3.scaleBand().range([0,h]).domain(s.map(e=>e.index)).padding(1);m.append("g").attr("transform","translate(0,0)").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const v=d3.scaleLinear().domain([0,d3.max(s,e=>e.value)]).range([n,0]);m.append("g").call(d3.axisLeft(v)).attr("display","none"),m.append("path").datum(s).attr("fill",function(e){return 0<s[0].value?"#0D9401":"#EB0C24"}).attr("stroke",function(e){return 0<s[0].value?"#0B7901":"#AB0416"}).attr("stroke-width",1.5).attr("d",d3.area().x(e=>p(e.index)).y0(v(0)).y1(e=>v(e.value))),i()}};looker.plugins.visualizations.add(visObject);
