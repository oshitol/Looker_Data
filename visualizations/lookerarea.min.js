...
const visObject={create:function(e,t){e.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(e,t,l,a,n,r){const o=a.fields.dimension_like,i=a.fields.measure_like[0];console.log("data",i);const s=[];e.forEach(function(a){const n=[];for(const e of o){if(null===a[e.name].value&&!l.show_null_points)break;n.push(a[e.name].value+"")}n.forEach(function(e,t){s.push({index:s.length,name:n.join("-"),value:+a[i.name].value})})}),t.innerHTML=`<lable>${i.label_short}</lable>
    <br>
    <lable style="font-weight:bold">${s[0].value.toLocaleString("en-US",{style:"currency",currency:"USD"})}
    </lable>
    <br>`,console.log("final chart data",s);e=Math.min(960,500)/2-40;const c=d3.scaleOrdinal().range(["#98abc5","#8a89a6","#7b6888","#6b486b","#a05d56"]),d=d3.select("#vis").append("svg").attr("width",960).attr("height",500).append("g").attr("transform","translate(480,250)");const u=d3.pie().value(function(e){return e.value});t=u(d3.entries({a:9,b:20,c:30,d:8,e:12}));d.selectAll("whatever").data(t).enter().append("path").attr("d",d3.arc().innerRadius(100).outerRadius(e)).attr("fill",function(e){return c(e.data.key)}).attr("stroke","black").style("stroke-width","2px").style("opacity",.7),r()}};looker.plugins.visualizations.add(visObject);
