...
const visObject={create:function(e,n){e.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(e,n,o,t,a,l){console.log("data",e),console.log("element",n),console.log("config",o),console.log("queryResponse",t),console.log("details",a);const i=t.fields.dimension_like,s=t.fields.measure_like[0],r=[];e.forEach(function(t){const a=[];for(const e of i){if(null===t[e.name].value&&!o.show_null_points)break;a.push(t[e.name].value+"")}a.forEach(function(e,n){r.push({index:r.length,name:a.join("-"),value:+t[s.name].value})})});var d,c="";for(d of e){var u=d[t.fields.dimensions[0].name];(c=LookerCharts.Utils.htmlForCell(u)).onclick=function(e){LookerCharts.Utils.openDrillMenu({links:u.links,event:e})};break}n.innerHTML=c;e=n.clientWidth,n=n.clientHeight;const f=d3.select("#vis").append("svg").attr("width","100%").attr("height","100%").append("g"),p=d3.scaleBand().range([0,e]).domain(r.map(e=>e.index)).padding(1);f.append("g").attr("transform","translate(0,0)").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const h=d3.scaleLinear().domain([0,d3.max(r,e=>e.value)]).range([n,0]);f.append("g").call(d3.axisLeft(h)).attr("display","none"),f.append("path").datum(r).attr("fill",function(e){return 0<r[0].value?"#0D9401":"#EB0C24"}).attr("stroke",function(e){return 0<r[0].value?"#0B7901":"#AB0416"}).attr("stroke-width",1.5).attr("d",d3.area().x(e=>p(e.index)).y0(h(0)).y1(e=>h(e.value))),l()}};looker.plugins.visualizations.add(visObject);
