...
const visObject={create:function(n,e){n.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(n,e,i,t,a,l){const r=t.fields.dimension_like,o=t.fields.measure_like[0],s=[];n.forEach(function(t){const a=[];for(const n of r){if(null===t[n.name].value&&!i.show_null_points)break;a.push(t[n.name].value+"")}a.forEach(function(n,e){s.push({index:s.length,name:a.join("-"),value:+t[o.name].value})})});var d,c="";for(d of n){var u=d[t.fields.dimensions[0].name];(c+=LookerCharts.Utils.htmlForCell(u)).onclick=function(n){LookerCharts.Utils.openDrillMenu({links:u.links,event:n})}}e.innerHTML=c,console.log("final chart data",s);n=e.clientWidth,e=e.clientHeight;const f=d3.select("#vis").append("svg").attr("width","100%").attr("height","100%").append("g"),p=d3.scaleBand().range([0,n]).domain(s.map(n=>n.index)).padding(1);f.append("g").attr("transform","translate(0,0)").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const h=d3.scaleLinear().domain([0,d3.max(s,n=>n.value)]).range([e,0]);f.append("g").call(d3.axisLeft(h)).attr("display","none"),f.append("path").datum(s).attr("fill",function(n){return 0<s[0].value?"#0D9401":"#EB0C24"}).attr("stroke",function(n){return 0<s[0].value?"#0B7901":"#AB0416"}).attr("stroke-width",1.5).attr("d",d3.area().x(n=>p(n.index)).y0(h(0)).y1(n=>h(n.value))),l()}};looker.plugins.visualizations.add(visObject);
