...
const visObject={create:function(t,e){t.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(t,e,l,n,a,o){console.log("data",t),console.log("element",e),console.log("config",l),console.log("queryResponse",n),console.log("details",a);const r=n.fields.dimension_like,i=n.fields.measure_like[0],s=[];t.forEach(function(n){const a=[];for(const t of r){if(null===n[t.name].value&&!l.show_null_points)break;a.push(n[t.name].value+"")}a.forEach(function(t,e){s.push({index:s.length,name:a.join("-"),value:+n[i.name].value})})});var d;for(d of t){var c=d[n.fields.dimensions[0].name];LookerCharts.Utils.htmlForCell(c).onclick=function(t){LookerCharts.Utils.openDrillMenu({links:c.links,event:t})};break}e.innerHTML=`<table>
    <tr>
      <td>${i.label_short}</td>
      <td></td><td></td>
    ...
    <br>`;t=e.clientWidth,e=e.clientHeight;const u=d3.select("#vis").append("svg").attr("width","100%").attr("height","100%").append("g"),f=d3.scaleBand().range([0,t]).domain(s.map(t=>t.index)).padding(1);u.append("g").attr("transform","translate(0,0)").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const h=d3.scaleLinear().domain([0,d3.max(s,t=>t.value)]).range([e,0]);u.append("g").call(d3.axisLeft(h)).attr("display","none"),u.append("path").datum(s).attr("fill",function(t){return 0<s[0].value?"#0D9401":"#EB0C24"}).attr("stroke",function(t){return 0<s[0].value?"#0B7901":"#AB0416"}).attr("stroke-width",1.5).attr("d",d3.area().x(t=>f(t.index)).y0(h(0)).y1(t=>h(t.value))),o()}};looker.plugins.visualizations.add(visObject);
