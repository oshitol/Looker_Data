...
const visObject={options:{first_option:{type:"string",label:"My First Option",default:"Default Value"},second_option:{type:"number",label:"My Second Option",default:42}},create:function(t,e){t.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(t,o,e,a,n,l){const i=a.fields.dimension_like,r=a.fields.measure_like[0];console.log("data",r);const s=[];let d=0;t.forEach(function(a){const n=[];for(const t of i){if(null===a[t.name].value&&!e.show_null_points)break;n.push(a[t.name].value+"")}n.forEach(function(t,e){d<+a[r.name].value&&(d=+a[r.name].value,o.innerHTML=`<h4>${r.name}</h4><h4>$${d}</h4>`),s.push({index:s.length,name:n.join("-"),value:+a[r.name].value})})}),console.log("final chart data",s);var c={top:0,right:30,bottom:70,left:-145},a=o.clientWidth,t=o.clientHeight;const p=d3.select("#vis").append("svg").attr("width",a+c.left+c.right).attr("height",t+c.top+c.bottom).append("g").attr("transform","translate("+c.left+","+c.top+")"),u=d3.scaleBand().range([0,a]).domain(s.map(t=>t.index)).padding(1);p.append("g").attr("transform","translate(0,"+t+")").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const f=d3.scaleLinear().domain([0,d3.max(s,t=>t.value)]).range([t,0]);p.append("g").call(d3.axisLeft(f)).attr("display","none"),p.append("path").datum(s).attr("fill","#cce5df").attr("stroke","#69b3a2").attr("stroke-width",1.5).attr("d",d3.area().x(t=>u(t.index)).y0(f(0)).y1(t=>f(t.value))),l()}};looker.plugins.visualizations.add(visObject);
