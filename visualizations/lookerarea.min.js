...
const visObject={create:function(e,n){e.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(e,n,i,t,a,l){const o=t.fields.dimension_like,r=t.fields.measure_like[0],s=[];e.forEach(function(t){const a=[];for(const e of o){if(null===t[e.name].value&&!i.show_null_points)break;a.push(t[e.name].value+"")}a.forEach(function(e,n){s.push({index:s.length,name:a.join("-"),value:+t[r.name].value})})});var d,c="";for(d of e){var u=d[t.fields.dimensions[0].name],c=LookerCharts.Utils.htmlForCell(u);console.log("row",d[t.fields.dimensions[0]]),c.onclick=function(e){LookerCharts.Utils.openDrillMenu({links:u.links,event:e})};break}n.innerHTML=c,console.log("final chart data",s);e=n.clientWidth,n=n.clientHeight;const f=d3.select("#vis").append("svg").attr("width","100%").attr("height","100%").append("g"),p=d3.scaleBand().range([0,e]).domain(s.map(e=>e.index)).padding(1);f.append("g").attr("transform","translate(0,0)").selectAll("text").attr("transform","translate(-10,10)rotate(-45)").attr("display","none");const h=d3.scaleLinear().domain([0,d3.max(s,e=>e.value)]).range([n,0]);f.append("g").call(d3.axisLeft(h)).attr("display","none"),f.append("path").datum(s).attr("fill",function(e){return 0<s[0].value?"#0D9401":"#EB0C24"}).attr("stroke",function(e){return 0<s[0].value?"#0B7901":"#AB0416"}).attr("stroke-width",1.5).attr("d",d3.area().x(e=>p(e.index)).y0(h(0)).y1(e=>h(e.value))),l()}};looker.plugins.visualizations.add(visObject);
