...
const visObject={create:function(t,e){t.innerHTML="<h1>Ready to render!</h1>"},updateAsync:function(t,e,l,n,a,o){console.log("data",t),console.log("element",e),console.log("config",l),console.log("queryResponse",n),console.log("details",a);const i=n.fields.dimension_like,r=n.fields.measure_like[0],s=[];t.forEach(function(n){const a=[];for(const t of i){if(null===n[t.name].value&&!l.show_null_points)break;a.push(n[t.name].value+"")}a.forEach(function(t,e){s.push({index:s.length,name:a.join("-"),value:+n[r.name].value})})});var d,c="";for(d of t){var u=d[n.fields.dimensions[0].name];(c=LookerCharts.Utils.htmlForCell(u)).onclick=function(t){LookerCharts.Utils.openDrillMenu({links:u.links,event:t})};break}e.innerHTML=`<table>
    ...
    <br>`;t=e.clientWidth,e=e.clientHeight;const f=d3.select("#vis").append("svg").attr("width","100%").attr("height","100%").append("g"),h=d3.scaleBand().range([0,t]).domain(s.map(t=>t.index)).padding(1);f.append("g").attr("transform","translate(0,"+e+")").call(d3.axisBottom(h).tickFormat(t=>s[t].name)).selectAll("text").attr("transform","translate(-10,10)rotate(-45)").style("text-anchor","end");const m=d3.scaleLinear().domain([0,d3.max(s,t=>t.value)]).range([e,0]);f.append("g").call(d3.axisLeft(m)),f.append("path").datum(s).attr("fill",function(t){return 0<s[0].value?"#0D9401":"#EB0C24"}).attr("stroke",function(t){return 0<s[0].value?"#0B7901":"#AB0416"}).attr("stroke-width",1.5).attr("d",d3.area().x(t=>h(t.index)).y0(m(0)).y1(t=>m(t.value))),o()}};looker.plugins.visualizations.add(visObject);
